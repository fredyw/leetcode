...
pub fn smallest_absent(nums: Vec<i32>) -> i32 {
    ...
    let mut avg = (nums.iter().sum::<i32>() / nums.len() as i32) + 1;
    if avg <= 0 {
        avg = 1;
    }
...
fn main() {
    ...
    println!("{}", smallest_absent(vec![89])); // 90
    println!("{}", smallest_absent(vec![-4, -3, -2])); // 1
    println!("{}", smallest_absent(vec![-34])); // 1
    println!("{}", smallest_absent(vec![1, -39, 9])); // 2
}

...
public class Problem1105 {
    public int minHeightShelves(int[][] books, int shelf_width) {
        return minHeightShelves(
                books, shelf_width, 0, 0, 0, new Integer[books.length][shelf_width + 1]);
    }

    private static int minHeightShelves(
            int[][] books,
            int shelfWidth,
            int totalWidth,
            int maxHeight,
        ...
        if (totalWidth + width <= shelfWidth) {
            ...
            answer =
                    minHeightShelves(books, shelfWidth, width, height, index + 1, memo) + maxHeight;
        }

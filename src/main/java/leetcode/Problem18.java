        ...
        for (int i = 0; i < num.length; i++) {
            for (int j = i+1; j < num.length; j++) {
                for (int k = j+1; k < num.length; k++) {
                    int expected = target - (num[i] + num[j] + num[k]);
                    if (map.containsKey(expected)) {
                        int l = map.get(expected);
                        if (l > k) {
                            List<Integer> list = Arrays.asList(num[i], num[j], num[k], expected);
